    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <div className="mx-auto max-w-7xl px-6 pb-24 pt-10 sm:px-10 lg:px-16">
        <header className="mb-10 space-y-4">
          <p className="text-sm uppercase tracking-[0.35em] text-sky-300/80">Rewards</p>
          <h1 className="text-4xl font-semibold text-white sm:text-5xl">Earn reputation for discovery.</h1>
          <p className="max-w-3xl text-lg leading-8 text-slate-400">Track your Trend Scout, Champion Hunter, Connector, and Validator achievements across the platform.</p>
        </header>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {badges.map((badge, index) => (
            <motion.div
              ...
              className="rounded-3xl border border-slate-800/80 bg-slate-900/80 p-6 text-center shadow-xl shadow-slate-950/20"
            >
              ...
              <p className="mt-2 text-sm text-slate-500">Earned {badge.count} times</p>
            </motion.div>
          ))}
        </div>
      </div>
