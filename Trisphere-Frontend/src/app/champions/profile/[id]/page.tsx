    ...
    <main className="min-h-screen bg-midnight text-foreground">
      <div className="mx-auto max-w-7xl px-6 pb-24 pt-10 sm:px-10 lg:px-16">
        <header className="mb-8 space-y-3">
          <Link href="/champions/discover" className="text-sm text-primary hover:text-white">
            ← Discover
          </Link>
          ...
          <>
            <div className="rounded-2xl border border-border/80 bg-card/80 p-6">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  <div className="text-sm text-muted">Validation</div>
                  <div className="mt-1 text-sm font-semibold text-primary">{data.validation_status || '—'}</div>
                </div>
              </div>
            </div>

            <section className="mt-6 rounded-2xl border border-border/80 bg-card/80 p-6">
              <h2 className="font-semibold text-white">Profile (markdown)</h2>
              <pre className="mt-4 whitespace-pre-wrap text-sm leading-7 text-foreground/80">{data.profile_markdown}</pre>
            </section>
          </>
        ) : null}
      </div>
